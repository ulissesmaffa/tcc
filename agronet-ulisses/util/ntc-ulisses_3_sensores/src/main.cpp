...
{
    printf("\nInicializando o sistema de arquivos SPIFFS");
    ...
    if (ret != ESP_OK)
    {
        if (ret == ESP_FAIL)
        {
            printf("\nFalha ao montar ou formatar o sistema de arquivos");
        }
        else if (ret == ESP_ERR_NOT_FOUND)
        {
            printf("\nPartição SPIFFS não encontrada");
        }
        else
        {
            printf("\nErro desconhecido ao inicializar SPIFFS: %s", esp_err_to_name(ret));
        }
    }
    else
    {
        ...
        {
            printf("\nSistema de arquivos montado, total: %d, usado: %d", total, used);
        }
    }
    return ret;
}
...
void write_to_file(const char *filename, const char *data)
{
    ...
    if (f == NULL)
    {
        printf("\nFalha ao abrir o arquivo %s para escrita", filename);
        return;
    }

    fprintf(f, "%s\n", data); // Escreve a linha no arquivo e adiciona uma quebra de linha
    fclose(f);
    printf("\nDados gravados no arquivo: %s", data);
}

void read_from_file(const char *filename)
{
    printf("\nread_from_file", "[%lld] Inicio", esp_timer_get_time());
    FILE *f = fopen(filename, "r");
    if (f == NULL)
    {
        printf("\nFalha ao abrir o arquivo %s para leitura", filename);
        return;
    }
    ...
    while (fgets(line, sizeof(line), f) != NULL)
    {
        ...
        printf("\nLinha [%i] lida: %s", i, line);
        i++; // Incrementa o contador para a próxima linha
    }

    fclose(f);
    printf("[%lld] Fim", esp_timer_get_time());
}
...
void get_file_info(const char *filename)
{
    ...
    if (f == NULL)
    {
        ...
        if (f == NULL)
        {
            printf("\nNão foi possível criar o arquivo %s", filename);
            return;
        }
        printf("\nArquivo %s criado", filename);
    }
    else
    {
        ...
        printf("\nget_file_info", "O arquivo %s contém %d linhas", filename, line_count);
    }
    fclose(f);
}
...
void delete_file(const char *filename)
{
    ...
    {
        printf("\ndelete_file", "Arquivo '%s' excluído com sucesso.", filename);
    }
    else
    {
        printf("\ndelete_file", "Falha ao excluir o arquivo '%s'.", filename);
    }
}
...
void setup()
{
  ...
  delay(1);
    ...
    {
        printf("\nErro ao inicializar o sistema de arquivos SPIFFS");
...
{
    while (1)
    {                                           // loop que irá coletar a temperatura de hora em hora.
        ...
        printf("\nAguardando para próxima amostra...");
